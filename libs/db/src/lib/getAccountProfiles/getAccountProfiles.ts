...
export function summarizeProfile(profile: Profile, display: DisplaySummary): ProfileSummary;
export function summarizeProfile(
  ...
  const { name, displayName, bio } = ("display" in profile ? profile.display : display) ?? {};
  ...
  const fullUsername =
    "parent" in profile && !!profile.parent
      ? UNIVERSAL_SUBDOMAIN || profile.parent.subdomain
